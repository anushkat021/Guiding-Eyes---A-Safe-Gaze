...
function speakText(text) {
    ...
    // Choose a gentle, sweet female voice
    const voices = synth.getVoices();
    const sweetVoice = voices.find(voice => voice.name.includes('Google UK English Female') || voice.name.includes('Samantha'));

    if (sweetVoice) {
        utterance.voice = sweetVoice;
    }
    
    // Softer tone, slow rate, gentle pitch
    utterance.pitch = 1.5;   // Slightly higher pitch for a sweeter tone
    utterance.rate = 0.9;    // Slow the rate for a softer and smoother delivery
...
// Speak Welcome or Predictions with female voice
function speakText(text) {
    const synth = window.speechSynthesis;
    const utterance = new SpeechSynthesisUtterance(text);
    
    // Choose a sweet female voice
    const voices = synth.getVoices();
    const femaleVoice = voices.find(voice => voice.name.includes('Female') || voice.name.includes('Google UK English Female') || voice.name.includes('Samantha'));
